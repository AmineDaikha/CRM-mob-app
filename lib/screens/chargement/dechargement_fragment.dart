...
class DechargementFragment extends StatefulWidget {
  final Client client;

  const DechargementFragment({super.key, required this.client});

  @override
  State<DechargementFragment> createState() => _DechargementFragmentState();
}

class _DechargementFragmentState extends State<DechargementFragment> {
  String result = '';
  double total = 0;
  int nbProduct = 0;
  int PageNumber = 1;
  ...
  Future<void> fetchData() async {
    ...
    http.Response req = await http.get(
        Uri.parse(AppUrl.articlesFromDepotPage +
            '${AppUrl.user.etblssmnt!.code}/${AppUrl.user.localDepot!.id}' + '?PageNumber=1&Filter=$filter&PageSize=20'),
        headers: {
