...
class _NewCommandPageState extends State<NewCommandPage> {
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              onPressed: () async {
                try {
                  AppUrl.user.localDepot!.id!;
                } catch (e) {
                  showAlertDialog(context, 'Vous n\'avez pas un depCode !');
                  return;
                }
                ...
                if (confirmed) {
                  ...
                  _getCurrentLocation().then((value) {
                    if (value) {
                      ...
                      else if (widget.client.command!.type == 'Commande')
                        widget.client.lib = 'Prise de commande';
                      print('type is : ${widget.client.command!.type}');
  ...
  //   };
  //   http.Response req =
  //   await http.post(Uri.parse(AppUrl.commands), body: jsonEncode(jsonObject), headers: {
  ...
  Future<void> sendDocument(BuildContext context) async {
    ...
    for (Product product in widget.client.command!.products) {
      int nbColis = (product.quantity / product.quantityOfColis!).toInt();
      int nbUnite = (product.quantity % product.quantityOfColis!).toInt();
      String codeColis =
          '$nbColis' + 'C' + '${product.quantityOfColis}' + ' $nbUnite' + 'U';
      Map<String, dynamic> jsonProduct = {
        "artCode": product.id,
        "lib": product.name,
