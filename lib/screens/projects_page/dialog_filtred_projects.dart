...
class FiltredProjectsDialog extends StatefulWidget {
  const FiltredProjectsDialog({
    super.key,
  });

  @override
  State<FiltredProjectsDialog> createState() =>
      _FiltredCommandsClientDialogState();
}

class _FiltredCommandsClientDialogState extends State<FiltredProjectsDialog> {
  ...
  void initState() {
    ...
    if(AppUrl.filtredCommandsClient.stepPipProject == null || AppUrl.filtredCommandsClient.stepPipProject!.id == -1){
      selectedStep = allSteps.first;
      AppUrl.filtredCommandsClient.stepPipProject = selectedStep;
    }
    else {
      selectedStep = allSteps.where((element) => element.id == AppUrl.filtredCommandsClient.stepPipProject!.id).first;
    }
  }
                ...
                GestureDetector(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        '${DateFormat('dd-MM-yy').format(selectedDateEnd)}',
                        style: Theme.of(context).textTheme.headline3,
                      ),
                    ],
                  ),
