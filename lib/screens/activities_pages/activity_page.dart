...
class _ActivityPageState extends State<ActivityPage> {
  ...
  void initState() {
    ...
    actionTypes = widget.allTypes
        ...
        .toList();
    print('lenght ${actionTypes.length}');
    ...
    if (widget.activity.emergency != null)
      ...
      ratingEmergency = 0.0;
    print('ratingEmergency $ratingEmergency');
                            ...
                            GestureDetector(
                                ...
                                onTap: () async {
                                  if(widget.activity.state == states[1]){
                                    showAlertDialog(context, 'Cette activité a déjà commencée');
                                    return;
                                  }
                            ...
                            GestureDetector(
                                ...
                                onTap: () async {
                                  ...
                                  if(widget.activity.state == states[4]){
                                    showAlertDialog(context, 'Cette activité a déjà annulée');
                                    return;
                                  }
                                  ...
                                  ConfirmationDialog confirmationDialog =
                                      ConfirmationDialog();
                                  bool confirmed = await confirmationDialog
                                      .showConfirmationDialog(context, 'endAct');
                                  if (confirmed) {
                                    showLoaderDialog(context);
  ...
  Future<bool> endActivity(BuildContext context, Activity activity) async {
    ...
    print('fdee : ${activity.res['users'][0]['ID']}');
    print('fdee : ${activity.res['tiers']}');
    print('obj: ${activity.res}');
    // activity.res['tiers'] = null;
    print('fdee : ${activity.res['tiers']}');
    String url = AppUrl.editAcivitiesOpp + activity.id!;
    http.Response req = await http
        .put(Uri.parse(url), body: jsonEncode(activity.res), headers: {
  ...
  void reload() {
    ...
    //widget.activity.res = res;
    setState(() {
      print('iss: ${oldActivity == widget.activity}');
